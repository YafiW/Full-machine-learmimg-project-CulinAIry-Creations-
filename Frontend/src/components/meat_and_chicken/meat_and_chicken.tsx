...
import './meat_and_chicken.scss';

const MeatAndChicken = () => {
  const [meatAndChicken, setmeatAndChicken] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    const getVegetablesData = async () => {
      ...
      ingredientsService.getMeatAndChicken().then((ingredients:any)=>{
        setmeatAndChicken(ingredients.data);
      })
    }
    getVegetablesData();
  }, []);
  ...
  return (
    <div className="meatAndChicken">
      <div>
        {meatAndChicken.map((image, index) => {
          ...
          return (
            <button
              ...
              className={`meatAndChickenImage ${arrIngredients.includes(imageName) ? 'overlay' : ''}`}
            >
              <h5>{imageName}</h5>
              ...
              <img
                className="meatAndChickenImageaa"
                src={`http://localhost:5000/ingredients/meat_and_chicken/${image}`}
                alt={`meatAndChicken ${imageName}`}
                width="150px"
                height="150px"
              />
            </button>
